...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
      ...
      // Initialize the memory control system
      MSC_Init();
      break;
      ...
      if (characteristic == gattdb_read_write) {
        // Write user supplied value
          if (evt->data.evt_gatt_server_attribute_value.value.data[0]) {
              uint8_t i =0;
              for(i = 0; i<4; i++){
                uint32_t data = evt->data.evt_gatt_server_attribute_value.value.data[i];
                userData[0] = userData[0] + (data<<(8*i));
                app_log_info("data[%d] = %x\n",i, evt->data.evt_gatt_server_attribute_value.value.data[i]);
                app_log_info("userData[0] = %x", userData[0]);
              }
              for(i = 4; i<8; i++){
                uint32_t data = evt->data.evt_gatt_server_attribute_value.value.data[i];
                userData[1] = userData[1] + (data<<(8*(i-4)));
                app_log_info("data[%d] = %x\n",i, evt->data.evt_gatt_server_attribute_value.value.data[i]);
                app_log_info("userData[1] = %x", userData[1]);
              }
          }
